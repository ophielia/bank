...
public class TransactionManagerDaoHib extends HibernateDaoSupport implements
		TransactionManagerDao {
	...
	public void deleteBankTA(Long todelete) {
		BankTADao bankta = getBankTA(todelete);
		bankta.setDeleted(new Boolean(true));

		getHibernateTemplate().saveOrUpdate(bankta);
	}

	public BankTADao getBankTA(Long transid) {
		BankTADao trans = (BankTADao) getHibernateTemplate().get(
				BankTADao.class, transid);
		return trans;
	}
	...
	public void createOrSaveCatTrans(CategoryTADao catta) {
		getHibernateTemplate().saveOrUpdate(catta);
	}
